...
#define DH_DEVSTATE_PTZ_LOCATION    0x0036      // 查询云台状态信息(对应结构体ALARM_PTZ_LOCATION_INFO)
...
#define DH_IPSEARCH_EVENT_EX        0x3005      // IP搜索事件, 返回字符串格式如下:DevName:::Manufacturer::MAC:: IP:: Port&& DevName:: Manufacturer::MAC:: IP:: Port&&…
...
{ 
	...
	DH_REMOTE_HOST		struDns;				// DNS服务器
	DH_MAIL_CFG			struMail;				// 邮件配置
	BOOL                bMcastEnable;           // 组播使能位，0,不使能，1使能
...
typedef struct __DEV_DECODER_INFO 
{
	...
	BYTE            reserved1[3];
	BYTE            byLayoutEnable[28];     // 各解码通道显示叠加信息使能
	char			reserved[32];
...
typedef struct tagDEV_EVENT_TRAFFIC_CROSSLANE_INFO 
{
	...
	BYTE                byReserved[3];           
	int                 nSpeed;                         // 车辆实际速度，km/h
	BYTE				bReserved[1016];				// 保留字节,留待扩展.留待扩展.
...
typedef struct tagDEV_EVENT_TRAFFIC_TRAFFICCAR_INFO
{
	...
	int                nVehicleSize;                    // 车辆大小	参见VideoAnalyseRule中车辆大小定义
	float              fVehicleLength;                  // 车辆长度	单位米
	//int                nDrivingDirection;               // 行驶方向	0-上行，即车辆离设备部署点越来越近；1-下行，即车辆离设备部署点越来越远
...
typedef int  (CALLBACK *fFluxStatDataCallBack)(LONG lFluxStatHandle, DWORD dwEventType, void* pEventInfo, BYTE *pBuffer, DWORD dwBufSize, DWORD dwUser, int nSequence, void *reserved);
...
// 视频监视断开回调函数原形, 
typedef void (CALLBACK *fRealPlayDisConnect)(LONG lOperateHandle, EM_REALPLAY_DISCONNECT_EVENT_TYPE dwEventType, void* param, DWORD dwUser);
...
// GPS温湿度信息订阅回调
typedef void (CALLBACK *fGPSTempHumidityRev)(LONG lLoginID, GPS_TEMP_HUMIDITY_INFO GpsTHInfo, DWORD dwUserData);
...
//打开实时监视，若返回0表示打开失败
CLIENT_API LONG CALL_METHOD CLIENT_StartRealPlay(LONG lLoginID, int nChannelID, HWND hWnd, DH_RealPlayType rType, fRealDataCallBackEx cbRealData, fRealPlayDisConnect cbDisconnect, DWORD dwUser, DWORD dwWaitTime = 10000);
...
// 大华云台控制
CLIENT_API BOOL CALL_METHOD CLIENT_DHPTZControl(LONG lLoginID, int nChannelID, DWORD dwPTZCommand, unsigned char param1, unsigned char param2, unsigned char param3, BOOL dwStop,void* param4=NULL);
...
//------------------------------------------------------------------------
// 查询某月的各天是否存在录像文件
CLIENT_API BOOL CALL_METHOD CLIENT_QueryRecordStatus(LONG lLoginID, int nChannelId, int nRecordFileType, LPNET_TIME tmMonth, char* pchCardid, LPNET_RECORD_STATUS pRecordStatus, int waittime=1000);
// 查询时间段内是否有录像文件
...
CLIENT_API BOOL CALL_METHOD CLIENT_QuickQueryRecordFile(LONG lLoginID, int nChannelId, int nRecordFileType, LPNET_TIME tmStart, LPNET_TIME tmEnd, char* pchCardid, LPNET_RECORDFILE_INFO nriFileinfo, int maxlen, int *filecount, int waittime=1000, BOOL bTime = FALSE);
...
// 按文件方式回放--回调回放中断事件
CLIENT_API LONG CALL_METHOD CLIENT_StartPlayBackByRecordFile(LONG lLoginID,  LPNET_RECORDFILE_INFO lpRecordFile, HWND hWnd, 
															 fDownLoadPosCallBack cbDownLoadPos, DWORD dwPosUser, 
															 fDataCallBack fDownLoadDataCallBack, DWORD dwDataUser, 
															 fRealPlayDisConnect fDisConnectCallBack, DWORD dwDisUser, DWORD dwWaitTime = 10000);
...
// 按时间方式回放--回调回放中断事件
CLIENT_API LONG CALL_METHOD CLIENT_StartPlayBackByTime(LONG lLoginID, int nChannelID, 
...
// 控制播放方向--正放或者倒放
CLIENT_API BOOL CALL_METHOD CLIENT_PlayBackControlDirection(LONG lPlayHandle, BOOL bBackward);
...
// 智能检索回放
CLIENT_API BOOL CALL_METHOD CLIENT_SmartSearchPlayBack(LONG lPlayHandle, LPIntelligentSearchPlay lpPlayBackParam);
...
CLIENT_API BOOL CALL_METHOD CLIENT_QueryExtraRecordState(LONG lLoginID, char *pRSBuffer, int maxlen, int *nRSBufferlen, void *pReserved, int waittime=1000);
...
// 设置通道辅码流录像状态
CLIENT_API BOOL CALL_METHOD CLIENT_SetupExtraRecordState(LONG lLoginID, char *pRSBuffer, int nRSBufferlen, void* pReserved);
...
// 抓图请求扩展接口
CLIENT_API BOOL CALL_METHOD CLIENT_SnapPictureEx(LONG lLoginID, SNAP_PARAMS *par, int *reserved = 0);
...
// 设置GPS温湿度订阅回调函数
CLIENT_API void CALL_METHOD CLIENT_SetSubcribeGPSTHCallBack(fGPSTempHumidityRev OnGPSMessage, DWORD dwUser);

// GPS温湿度信息订阅
CLIENT_API BOOL CALL_METHOD CLIENT_SubcribeGPSTempHumidity (LONG lLoginID, BOOL bStart,	int InterTime, void* Reserved);

//GPS日志信息查询
CLIENT_API BOOL CALL_METHOD CLIENT_QueryGPSLog(LONG lLoginID,QUERY_GPS_LOG_PARAM *pQueryParam, char *pLogBuffer, int nLogBufferLen, int *pRecLogNum, BOOL *bContinue, int waittime);
...
CLIENT_API BOOL CALL_METHOD CLIENT_SetDecLayOutEnable(LONG lLoginID, BYTE bDecLayOutEnable, int nChannel, int waittime=1000);
//------------------------------------------------------------------------
...
CLIENT_API LONG CALL_METHOD CLIENT_RealLoadPictureEx(LONG lLoginID, int nChannelID, 
													 DWORD dwAlarmType, 
													 BOOL bNeedPicFile, 
...
// 下载离线智能分析数据－图片
CLIENT_API LONG CALL_METHOD CLIENT_LoadOffLineFile(LONG lLoginID, int nChannelID, DWORD dwAlarmType, LPNET_TIME_EX lpStartTime, LPNET_TIME_EX lpEndTime, fAnalyzerDataCallBack cbAnalyzerData, DWORD dwUser);

// 交通抓拍--网络触发方式
CLIENT_API BOOL CALL_METHOD CLIENT_TrafficSnapByNetwork(LONG lLoginID, int nChannelID, NET_IN_SNAPSHOT* pstInParam, NET_OUT_SNAPSHOT* pstOutParam);
// 交通流量统计
CLIENT_API LONG CALL_METHOD CLIENT_StartTrafficFluxStat(LONG lLoginID, NET_IN_TRAFFICFLUXSTAT* pstInParam,NET_OUT_TRAFFICFLUXSTAT* pstOutParam);

// 结束流量统计
CLIENT_API BOOL CALL_METHOD CLIENT_StopTrafficFluxStat(LONG lFluxStatHandle);

// 获取流量统计信息
CLIENT_API LONG CALL_METHOD CLIENT_StartFindFluxStat(LONG lLoginID, NET_IN_TRAFFICSTARTFINDSTAT* pstInParam, NET_OUT_TRAFFICSTARTFINDSTAT* pstOutParam);

// 继续查询流量统计
CLIENT_API int	CALL_METHOD CLIENT_DoFindFluxStat(LONG lFindHandle, NET_IN_TRAFFICDOFINDSTAT* pstInParam,NET_OUT_TRAFFICDOFINDSTAT* pstOutParam);

// 结束查询流量统计
CLIENT_API BOOL CALL_METHOD CLIENT_StopFindFluxStat(LONG lFindHandle);
///////////////////////////////特殊版本接口///////////////////////////////
...
// web信息上传接口
CLIENT_API BOOL  CALL_METHOD CLIENT_TransmitInfoForWeb(LONG lLoginID, char* szInBuffer, DWORD dwInBufferSize, char* szOutBuffer, DWORD dwOutBufferSize, void* pReserved, int waittime=500);
