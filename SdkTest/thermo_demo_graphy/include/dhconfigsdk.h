...
#define CFG_CMD_GPS_LOCATION_VER    "GPSLocationVersion"   // 记录集GPSLocation的版本号(对应CFG_LOCATION_VER_INFO)
...
//音频输出通道对应的InputChannel信息
typedef struct tagCFG_AUDIO_MATRIX_LIST
{
    BOOL bIsEnable;        // 使能，表示当前输出通道是否使用   
    int  nChannelCount;   // 对应的输入通道数量
    int  InputChannel[CFG_MAX_AUDIO_MATRIX_INPUT]; // 输入通道
} CFG_AUDIO_MATRIX_LIST;

//音频矩阵配置信息
typedef struct tagCFG_AUDIO_MATRIX
{
    CFG_AUDIO_MATRIX_LIST stAudioMtrixList[CFG_MAX_AUDIO_MATRIX_NUM][CFG_MAX_AUDIO_MATRIX_OUTPUT]; // 音频输出通道列表
} CFG_AUDIO_MATRIX;
